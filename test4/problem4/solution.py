...
# We can move across a cell only if we have positive points ( > 0 ). Whenever we pass through a cell, points in that
# cell are added to our overall points. We need to find minimum initial points to reach cell (m-1, n-1)
# from (0, 0) by following these certain set of rules :
#
...
def solve(matrix, r, c):
    ...
    for j in range(c-2, -1, -1):
        dp[r-1][j] = max(dp[r-1][j+1] - matrix[r-1][j-1], 1)

    for i in range(r-2, -1, -1):
        for j in range(c-2, -1, -1):
            dp[i][j] = max(min(dp[i+1][j], dp[i][j+1]) - matrix[i][j], 1)
