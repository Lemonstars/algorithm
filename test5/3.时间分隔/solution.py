...
def solve(arrival, departure):
    ...
    while arrival:
        ...
        while True:
            ...
            d_min = sys.maxsize
            d_pre = departure[cur_del]
            for i in range(cur_del+1, len(departure)):
                if arrival[i] > d_pre and departure[i] < d_min:
                    cur_del = i
                    d_min = departure[i]
                    more = True
            if more:
                del_list.append(cur_del)
            else:
                break

        count += 1
        for i in range(len(del_list)-1, -1, -1):
            del arrival[del_list[i]]
            del departure[del_list[i]]
